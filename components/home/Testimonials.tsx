...
export default async function Testimonials(props: LanguagePropsType) {
    const { lng } = props
    const { t } = await useTranslation(lng, 'testimonials')
    return (
        <div className={styles.testimonials}>
            {testimonialItems.map((item, index) => {
                return <div key={'testimonial'+index} className={styles.testimonial}>
                    <p>{t(item.testimonial)}</p>
                    <Image className={styles.logo} src={item.src} width={item.width} height={item.height} alt={item.alt}/>
                </div>
            })}
        </div>
    )
