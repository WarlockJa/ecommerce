...
import { CartItemType, LiteralLanguages } from "@/index";
...
const handleCartShownSwitch = (props: {
  cartShown: boolean;
  setCartShown: (state: boolean) => void;
...
export default function Cart({ lng }: { lng: LiteralLanguages }) {
  ...
  return (
    <>
      <MyButton
        ...
        onClick={() =>
          handleCartShownSwitch({ cartShown, setCartShown, scrollbarWidth })
        }
      >
        {cartEmpty ? (
      ...
      <section className={styles.cartOverlay} style={shopHiddenStyle}>
        <div
          style={{}}
        ...
        <div className={styles.cartSection}>
          ...
          <div className={styles.cartBody}>
            {cartEmpty ? (
              <Fragment>
                <div className={styles.cartEmpty}>{t("cart-empty")}</div>
                <DividerLine />
              </Fragment>
            ) : (
              cartItems
          ...
          <div className={styles.cartFooter}>
            <div className={styles.cartTotal}>
              {t("cart-total")}: {Math.abs(cartTotal).toFixed(2)}$
            </div>
            <MyButton
              style={defaultButtonStyle}
              className={styles.cartButton}
              onClick={() => handleCheckout({ cartItems: cartItems, lng: lng })}
            >
              {t("cart-buybutton")}
            </MyButton>
          </div>
